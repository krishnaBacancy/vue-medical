...
export default {
  ...
  data() {
    return {
      getAdminId: localStorage.getItem("user_id"),
  ...
  computed: {
    ...mapGetters("doctors", ["getAllDoctorsOnly", "loadingStatus"]),
  },
  methods: {
    ...mapActions("doctors", ["getAllPatientsData", "deletePatient"]),
    async deleteSingleDoctor(item) {
      if (
        ...
        this.deletePatient(item)
          .then((data) => {
            if (data.statusCode === 200) {
              setTimeout(() => {
                this.getAllPatientsData(this.getAdminId);
              }, 500);
